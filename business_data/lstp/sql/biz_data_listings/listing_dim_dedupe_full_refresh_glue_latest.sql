...
(
select
row_number() OVER (PARTITION BY listingid ORDER BY cast(listingdimkey as bigint) DESC) AS row_number_seq,
isaolsyndication,
masterplanid,
specid,
...
lower(edw_usps_geo_data.City) geo_city,
lower(edw_usps_geo_data.CityID) geo_city_id,
lower(edw_usps_geo_data.Country) geo_country,
lower(edw_usps_geo_data.CountryID) geo_country_id,
lower(edw_usps_geo_data.County) geo_county,
lower(edw_usps_geo_data.CountyFIPS) geo_county_fips,
lower(edw_usps_geo_data.CountyID) geo_county_id,
lower(edw_usps_geo_data.CountyType) geo_county_type,
lower(edw_usps_geo_data.CountyTypeID) geo_county_type_id,
lower(edw_usps_geo_data.State) geo_state,
lower(edw_usps_geo_data.StateFIPS) geo_state_fips,
lower(edw_usps_geo_data.StateID) geo_state_id,
...
edw_listings.listingagentofficedatasourceid mls_id,-->Modified
edw_listings.listingsourcealiasid mls_property_id, -->Modified
...
edw_properties_deduped.property_latitude property_latitude,
edw_properties_deduped.property_longitude property_longitude,
edw_listings.specid spec_id,
edw_listings.specialmessage special_message,
edw_listings.udblistingid udb_listing_id
FROM edw_listings
left join edw_properties_deduped ON edw_listings.propertyid=edw_properties_deduped.property_id
left join edw_usps_geo_data ON edw_listings.listingpostalcode=edw_usps_geo_data.PostalCode
left join listing_dim_keys_historical on listing_dim_keys_historical.listing_dim_key = edw_listings.listingdimkey
WHERE edw_listings.row_number_seq = 1
AND COALESCE(listing_dim_keys_historical.listing_dim_key,'')=''
and edw_listings.effective_to_datetime > '2023-01-01'
