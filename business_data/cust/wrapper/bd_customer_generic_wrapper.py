#Call python bd_customer_generic_wrapper.py --args '{"--APP_ID": "cust.cdh.bd.customer_party"}'
...
def main():
    ...
    try:
        ...
        for source in app_config["source"]:
            i = {"s3_source_path":source["s3_source_path"],"source_file_type_parquet_or_text":source["source_file_type_parquet_or_text"],"temp_table_name":source["temp_table_name"]}
    ...
    # check if source app ids are locked
    ...
    var_args = {"--JOB_NAME": jobname,
                ...
                "--inputvariables": json.dumps(inputvariables).replace('"',"'").replace(' ',''),
                ...
                "--data_frame_sources": json.dumps(data_frame_sources).replace('"',"'").replace(' ',''),
                "--target_partition_by_column": target_partition_by_column}
    print var_args
    print json.dumps(inputvariables).replace('"',"'").replace(' ','')
    print json.dumps(data_frame_sources).replace('"',"'").replace(' ','')
    print str(inputvariables)
    print str(data_frame_sources)
