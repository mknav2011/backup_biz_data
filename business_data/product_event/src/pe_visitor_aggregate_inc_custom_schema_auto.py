...
class AthenaCTAS(object):
    ...
    def __init__(self, sql_query=None ,target_table_name=None, env =None, s3_target_folder_parquet=None, 
        ...
        if glue_script_location is None:
            ...
            self.glue_script_location = 's3://move-dataeng-temp-%s/glue-etl/scripts/athena_ctas_csv2parquet.py' %(self.env)
        else:
            self.glue_script_location = glue_script_location
            
        if glue_custom_schema_location is None:
            self.glue_custom_schema_location = 's3://move-dataeng-temp-%s/glue-etl/schema_files/ctas_custom_schema.json' %(self.env)
        else:
...
def main():
    ...
    parser.add_argument('--env',  help='Execution Environment (dev, qa, prod) ')
