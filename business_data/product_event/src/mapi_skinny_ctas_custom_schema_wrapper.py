from move_dl_common_api.athena_ctas import AthenaCTAS
ctas_sql = """
...
,(case when lower(trim(params__querystring__src)) in ( 'rdc-android' , 'rdc-iphone','rdc-ipad','rdc-express-iphone','rdc-express-ipad','rdc-applewatch','rentals-iphone','rentals-ipad','rentals-ipad') then 'mobile apps'
else
(case
when lower(trim(params__querystring__src)) = 'rdc-android-instant' then 'android instant apps'
else 'others'
...
left outer join biz_data.event_type_dim b
...
AND day = '15'
"""
ctas = AthenaCTAS(sql_query=ctas_sql
                          ,target_db_name = 'prashi'
                          , target_table_name='product_event_mapi_biz_data'
