...
## Athena-CTAS-Executor
# Refactored version: v6: Default Arguments support; Only SQL File mandatory input
# Added Glue Crawler option to create Athena table on Parquet data created in ctas_views_db
# Added Start/EndTime metrics
# Wrapper for CTAS Class:
# Mandatory Input: Local location of SQL File, absolute filename
# Optional Input: Target S3 location
#TODO: Option to change DPUs
...
debug = 0
def ctas_process_wrapper(sql_file=None, input_date=None, s3_target_folder_parquet=None ):
    ...
    if s3_target_folder_parquet is None:
        ...
        update_start_and_wait_for_crawler_to_complete(crawler_name, ctas.s3_target_folder_parquet)
        print 
        text =  'Athena Table = %s is ready for use in Databse = %s \n S3 Location: %s' %(target_table_name, db_name, ctas.s3_target_folder_parquet)
        print banner(text)
        print banner('-')  
    print 
...
#sql_file = '/Users/apillai/move-git/athena-ctas/product_event/src/hiren_visitor_aggregate.sql'
sql_file = ar.sql_file # '/Users/apillai/move-git/athena-ctas/product_event/src/hiren_visitor_aggregate.sql'
s3_target_folder_parquet = None
#input_date = '2017-11-10'
input_date = None
#s3_target_folder_parquet = 's3://move-dataeng-temp-dev/glue-etl/parquet_data/product_event/hiren_visitor_aggregate_partioned'
#ctas_process_wrapper(sql_file=sql_file, input_date=input_date, s3_target_folder_parquet = '%s/event_date=%s' %(s3_target_folder_parquet, input_date))
s3_target_folder_parquet = None # 's3://move-dataeng-temp-dev/glue-etl/parquet_data/product_event/hiren_visitor_aggregate_partioned'
ctas_process_wrapper(sql_file=sql_file )
if s3_target_folder_parquet:
        crawler_name = 'ctas_views_crawler'
        db_name = 'ctas_views_db'
        update_start_and_wait_for_crawler_to_complete(crawler_name, s3_target_folder_parquet)
        print 
        text =  'Athena Table   is ready for use in Database = %s \n S3 Location: %s' %(  db_name, s3_target_folder_parquet)
        print banner(text)
        print banner('-')  
print 
