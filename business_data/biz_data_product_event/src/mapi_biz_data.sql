...
,a.params__querystring__visitor visitor_id_raw
,lower(trim(a.params__querystring__visitor)) visitor_id
,a.params__querystring__session visit_id_raw
,lower(trim(a.params__querystring__session)) visit_id
,params__querystring__chnl property_status_raw
,params__querystring__page page_type
,params__querystring__pgvar page_type_raw
...
when lower(trim(params__querystring__chnl)) = 'fcls' then 'foreclosure'
when lower(trim(params__querystring__chnl)) = 'nhms' then 'new plan'
ELSE 'not available' 
END) property_status_sub
...
,params__querystring__accid account_id_raw
,lower(trim(params__querystring__accid)) account_id
,params__querystring__adtsid advertiser_id_raw
, case when regexp_extract(params__querystring__adtsid, '[a].(\d+)', 1) is null
and  lower(trim(params__querystring__adtsid)) <> 'null'
and  REGEXP_LIKE(trim(params__querystring__adtsid), '^[[:digit:]]+$')
then trim(params__querystring__adtsid)
else regexp_extract(params__querystring__adtsid, '[a].(\d+)', 1)
...
,params__querystring__schid search_id_raw
,lower(trim(params__querystring__schid)) search_id
,params__querystring__lnkel social_shares
, (case when length(trim(params__querystring__lid)) = 9 then params__querystring__lid else null end) listing_id
, (case when length(trim(params__querystring__lid)) = 10 then params__querystring__lid else null end) mpr_id
, lower(trim(params__querystring__prop_id)) property_id
