...
def prepare_sql(lookup_tablename):
    ...
    WITH data as(
    select distinct    
     concat(regexp_split(split(reverse(split(reverse(input_filename) ,'/')[1]), '.')[1] , '-|_')[2]
    ,regexp_split(split(reverse(split(reverse(input_filename) ,'/')[1]), '.')[1] , '-|_')[3]
            ) as created_seq
    , *
    from  {table_name}  
    ), ranked as(
    select *, row_number() over(partition by col_00 order by created_seq desc) as rnk
    from data
    )
    select distinct col_00 as "mobile_id",
